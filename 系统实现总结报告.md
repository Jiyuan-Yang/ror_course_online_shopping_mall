...
用?来代替具体的值可以防止SQL注入攻击

#### 使用rails查询接口

rails提供了`where`、`order`、`select`、`group`、`having`等方法对于SQL中的对应语句，同时可以方便的使用ruby中的键值对作为参数而不是像上一种方式中的插入SQL。
...
类似于SQL中的外键，rails中的两个模型如果声明的相应的关系，可以通过一个对应的方法来之间访问到所有通过外键关联的对象，再利用ruby进行迭代，甚至可以做到在不使用SQL的情况下方便的进行数据库查询。
...
rails中表的建立和结构的修改都通过`db:migrate`命令和对应的migrate文件来实现，方便实现数据库表的建立、修改和回滚。

创建表：![](report_pic/7.png)
...
rails框架中通过`validate`方法方便的实现完成性约束。
...
通过`has_one`、`has_many`、`belong_to`等方法可以声明实体之间的外键约束关系，这样不仅可以在数据库的表中建立相应的约束，rails框架还会为对应的模型添加相应的方法，具有外键约束的对象之间可以方便的进行访问。

![](report_pic/11.png)

### CRUD

查询在第一部分说明过。剩下三种方法与查询类似，通过`create`、`update`、`destroy`
...
- 路由
  ...
  ... ...
  get '/sign_up', to: 'users#new'	# 响应get请求
  post '/sign_up', to: 'users#create'	# 响应post请求，用于表单的提交与用户的创建
  ... ...
